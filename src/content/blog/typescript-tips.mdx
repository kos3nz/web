...
type GetPoint<T> = T extends ComplexInterface<any, any, any, infer TPoint>
  ? TPoint
  : never
...
type GetLastName<T> = T extends `${string} ${infer TLastName}`
  ? TLastName
  : never
...
type UserNameGetters = {
  [K in keyof User as `get${Capitalize<
    Extract<K, `${string}Name`>
  >}`]: () => User[K]
}
/*
type UserNameGetters = {
